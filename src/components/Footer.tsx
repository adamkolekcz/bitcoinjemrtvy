import Link from "next/link";
import { getBitcoinAgeYears } from "@/lib/calculations";
import { FooterBtcPrice } from "@/components/FooterBtcPrice";

const GITHUB_URL = "https://github.com/adamkolekcz/bitcoinjemrtvy";
const LICENSE_URL = "https://github.com/adamkolekcz/bitcoinjemrtvy/blob/main/LICENSE";

const linkClass =
  "text-neutral-400 underline-offset-4 transition-colors hover:text-[var(--bitcoin-orange)] hover:underline";

export function Footer() {
  const years = getBitcoinAgeYears();
  const year = new Date().getFullYear();
  return (
    <footer className="bg-[var(--background)] py-12 text-sm">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          {/* Brand + teze */}
          <div className="sm:col-span-2">
            <Link href="/" className="inline-block">
              <span className="text-xl font-bold tracking-tight">
                <span className="text-[var(--bitcoin-orange)]">Bitcoin</span>{" "}
                <span className="text-white">je mrtvý</span>
              </span>
            </Link>
            <p className="mt-3 max-w-sm leading-relaxed text-neutral-400">
              Již {years}&nbsp;let sledujeme, jak Bitcoin umírá
            </p>
            <div className="mt-4">
              <FooterBtcPrice />
            </div>
          </div>

          {/* Navigace po webu */}
          <nav aria-label="Stránky webu">
            <h2 className="text-sm font-medium uppercase tracking-wider text-neutral-300">
              Na webu
            </h2>
            <ul className="mt-4 space-y-2.5">
              <li>
                <Link href="/" className={linkClass}>
                  Hlavní stránka
                </Link>
              </li>
              <li>
                <Link href="/prohlaseni" className={linkClass}>
                  Seznam nekrologů
                </Link>
              </li>
              <li>
                <Link href="/embed" className={linkClass}>
                  Widgety pro web
                </Link>
              </li>
            </ul>
          </nav>

          {/* Projekt a zdroje */}
          <nav aria-label="Projekt a zdroje">
            <h2 className="text-sm font-medium uppercase tracking-wider text-neutral-300">
              Projekt
            </h2>
            <ul className="mt-4 space-y-2.5">
              <li>
                <a
                  href={GITHUB_URL}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="Zdrojový kód na GitHubu (otevře se v novém okně)"
                  className={`inline-flex items-center gap-1.5 ${linkClass}`}
                >
                  <GithubIcon />
                  Zdrojový kód
                </a>
              </li>
              <li>
                <a
                  href="https://bitcoindeaths.com"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="Zdroj dat Bitcoinisdead (otevře se v novém okně)"
                  className={`inline-flex items-center gap-1 ${linkClass}`}
                >
                  Zdroj dat: Bitcoinisdead
                  <ArrowUpRight />
                </a>
              </li>
              <li>
                <a
                  href="https://www.linkedin.com/in/adamkolek/"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label="Autor Adam Kolek na LinkedIn (otevře se v novém okně)"
                  className={`inline-flex items-center gap-1 ${linkClass}`}
                >
                  Autor: Adam Kolek
                  <ArrowUpRight />
                </a>
              </li>
            </ul>
          </nav>
        </div>

        {/* Bottom bar */}
        <div className="mt-10 flex flex-col gap-2 border-t border-[var(--card-border)] pt-6 text-neutral-500 sm:flex-row sm:items-center sm:justify-between">
          <p>
            © {year} Adam Kolek · Kód pod{" "}
            <a
              href={LICENSE_URL}
              target="_blank"
              rel="noopener noreferrer"
              className={linkClass}
            >
              licencí MIT
            </a>
          </p>
          <p>Není investiční doporučení — web slouží výhradně k&nbsp;edukaci.</p>
        </div>
      </div>
    </footer>
  );
}

function GithubIcon() {
  return (
    <svg
      width="16"
      height="16"
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden="true"
      className="shrink-0"
    >
      <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23a11.5 11.5 0 0 1 3-.405c1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
    </svg>
  );
}

function ArrowUpRight() {
  return (
    <svg
      width="12"
      height="12"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2.5"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden="true"
      className="shrink-0 opacity-60"
    >
      <path d="M7 17 17 7M9 7h8v8" />
    </svg>
  );
}
