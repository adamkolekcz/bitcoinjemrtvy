...
export function buildEmbedSnippet(key: EmbedWidgetKey): string {
  ...
  return `<iframe src="${EMBED_ORIGIN}/embed/${w.key}" width="${w.width}" height="${w.height}" frameborder="0" scrolling="no" style="border:none;overflow:hidden;border-radius:12px;" title="${w.title}"></iframe>`;
}
